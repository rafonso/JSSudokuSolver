...
function postMessageToView(type, params) {
    /*
    postMessage(_.extend({
        type: type,
    }, extras));
    */
    actionByMessageFromSolver[type](params);
}
...
function changePuzzleStatus(status, extras) {
    ...
    // postMessage(message);
    postMessageToView(MessageFromSolver.PUZZLE_STATUS, _.extend({
        status: puzzle.status
    }, extras));
}
