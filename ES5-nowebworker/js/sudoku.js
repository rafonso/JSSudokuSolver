...
function notifyCellValue(row, col, value) {
    actionByMessageToSolver[MessageToSolver.FILL_CELL]({
row: row, //
        col: col, //
        value: value
    });
...
function postMessageToSolver(type, value) {
    actionByMessageToSolver[type](value);
    /*
...
function initComponents() {
    ...
    $("#btnRun").button("option", "icons", {
        ...
        function () {
            postMessageToSolver( MessageToSolver.START);
            // worker.postMessage({
    ...
    $("#btnClean").button("option", "icons", {
        ...
        function () {
            postMessageToSolver( MessageToSolver.CLEAN);
            // worker.postMessage({
    ...
    $("#btnStop").button("option", "icons", {
        primary: "ui-icon-stop"
            ...
            postMessageToSolver( MessageToSolver.STOP);
        // worker.postMessage({
    ...
    $("#btnReset").button("option", "icons", {
        primary: "ui-icon ui-icon-arrowrefresh-1-w"
            ...
            postMessageToSolver( MessageToSolver.RESET);
        // worker.postMessage({
    ...
    $("#steptime").selectmenu({
        select: function (event, ui) {
            postMessageToSolver( MessageToSolver.STEP_TIME, parseInt(ui.item.label, 10));
            // worker.postMessage({
